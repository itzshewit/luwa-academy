...
export const CinematicConcepts: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Tracking multimedia view activity
      updateProgress(activeSession.id, 'viewMultimedia', { mediaId: `${selectedSubject}_${topic}` });
  ...
  return (
    <div className="h-full flex flex-col gap-6 animate-fade-in">
      <div className="flex justify-between items-end mb-2">
        <div>
          <h2 className="text-2xl font-black uppercase tracking-tighter text-luwa-primary">Cinematic Synthesis</h2>
        ...
        <div className="bg-white px-4 py-2 rounded-xl border border-slate-100">
           ...
           <p className="text-[10px] text-luwa-primary font-black tracking-widest uppercase">720P • 16:9</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-hidden">
        <GlassCard className="flex flex-col gap-6 p-8 border-slate-100 bg-white">
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:outline-none focus:border-luwa-primary transition-all"
              />
            </div>
            <div className="w-full md:w-64">
              ...
              <select 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold focus:outline-none focus:border-luwa-primary transition-all appearance-none cursor-pointer"
              >
          ...
          <button 
            ...
            className="w-full bg-luwa-primary text-white font-black py-6 rounded-2xl text-xs uppercase tracking-[0.3em] shadow-xl shadow-luwa-primary/10 hover:brightness-110 disabled:opacity-20 transition-all"
          >
        ...
        <div className="flex-1 rounded-3xl relative overflow-hidden flex items-center justify-center min-h-[400px] border border-slate-100 bg-slate-50 shadow-inner">
          {loading ? (
            <div className="text-center p-10 animate-fade-in">
              <div className="w-12 h-12 border-2 border-slate-100 border-t-luwa-primary rounded-full animate-spin mx-auto mb-6" />
              <p className="text-luwa-primary font-black uppercase tracking-[0.2em] text-sm mb-3">{loadingMessages[msgIdx]}</p>
